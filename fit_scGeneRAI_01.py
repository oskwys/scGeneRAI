...
data = pd.concat((df_exp, df_mut_scale, df_amp, df_del, df_fus_scale), axis = 1)
...
if split_by_subtypes:
    ...
    for subtype in subtypes:
        ...
        for model_i in range(n_models):
            ...
            testlosses, trainlosses, epoch_list = model.fit(data_temp, nepochs, model_depth, lr=2e-2, batch_size=5, lr_decay = 0.99, descriptors = None, early_stopping = True, device_name = device)
            print('\n\n __________________ MODEL TRAINED ! __________________\n')
            
            # %% saving the model
            
            file_name = 'model_{}_batch{}_nepochs{}_depth{}_lr{}_i{}.pkl'.format(subtype, batch_size, nepochs, model_depth, str(learning_rate).replace('0.', ''), model_i)

            with open(file_name, 'wb') as file:
                pickle.dump(model, file)
                print(f'Object successfully saved to "{file_name}"')
        
            pd.DataFrame([testlosses, trainlosses, epoch_list]).T.to_csv('model_{}_batch{}_nepochs{}_depth{}_lr{}_i{}_losses.csv'.format(subtype, batch_size, nepochs, model_depth, str(learning_rate).replace('0.', ''), model_i))
